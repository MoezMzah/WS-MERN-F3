...
const login = async (req, res) => {
  ...
  try {
    ...
    if (!isMatch) {
       res.status(400).send([{ msg: "bad credential (password)" }]);
    }
...
const getAuthUser = (req, res) => { //getAuthUser is a function that take the token in header request and return the user of that token
